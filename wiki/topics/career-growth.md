...
**일상의 불편함에서 출발해 도구를 만드는 것을 즐기는, 소통 능력이 강한 AI 스타트업 프론트엔드 개발자.**

블로그의 가장 큰 축. 학생에서 AI 스타트업 프론트엔드 개발자로 성장하는 과정을 기록한다. 회고 시리즈를 통해 매년 자신을 돌아보며, 동료 평가와 자기 분석을 통해 성장 방향을 잡아간다.
...
동료 평가에서 모든 시기, 모든 직군(FE/BE/디자이너/PM)에 걸쳐 "소통과 공유를 잘한다"가 일관되게 나온다. 특히 기술적 개념을 비전공자가 이해할 수 있는 언어로 번역하는 능력이 있다. 개발자의 일을 "세계관을 만들고 관리하는 일"로 비유하거나, V8의 TimSort나 OKLCH 같은 복잡한 개념을 단계별 시각 예시로 설명하는 포스트들이 이를 보여준다.
...
"엘리스의 토끼굴"을 즐긴다. 근본적인 "왜?"에서 출발해 끝까지 파고드는 스타일. V8 sort를 파다가 TimSort의 피보나치 스택까지, OKLCH를 파다가 원추세포의 반응 곡선까지 들어간다.

### AI에 대한 입장

"개발자의 정의가 바뀔 뿐 사라지지 않는다." 바이브코딩이 아닌 증강코딩을 지향하며, "feature는 AI에게, 리팩토링과 설계는 내가" 하는 워크플로우를 정립 중.

### 현재 성장 방향

"재미 추구 → 방향 설정 → 설계 역량 강화"로 진화 중. 기초(디자인 시스템, 접근성)는 충분히 다졌고, 설계/아키텍처 역량을 키우는 단계로 넘어가려 한다. "바퀴를 재발명하는 경향"이라는 약점도 인식하고 있다.
...
**2024 초**: 인턴 3곳을 거치며 "정적인 환경은 맞지 않는다"는 자기 인식 → 스타트업 선택.

**2024 말**: "뭘 잘하고 싶은지 몰랐다"에서 "에디터 만드는 게 가장 즐겁다"로 방향 발견. 동료에게 소통 능력을 일관되게 인정받음.

**2025 상반기**: 설계 역량 부족, 바퀴 재발명 경향이라는 개선점 인식. AI 시대에 설계 능력의 가치가 올라감을 체감.
