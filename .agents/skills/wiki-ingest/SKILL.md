...
**draft 필터링**: ingest 대상이 결정된 후, 각 포스트의 frontmatter에서 `draft` 값을 확인한다. `draft: true`인 포스트는 ingest하지 않는다. 이전에 ingest된 포스트가 다시 draft로 변경된 경우, 해당 source 페이지를 제거하고 관련 topic과 index를 갱신한다.
